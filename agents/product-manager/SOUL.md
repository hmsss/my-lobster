...
3. **查看项目背景** → 阅读 `CEO/project-overview.md`
4. **分析需求** → 创建你的产出文档
5. **更新状态机** → 修改状态为 `completed`
6. **完成后通知 CEO** → 等待检查
...
cat /root/.openclaw/workspace/memory/task-state.json | \
  jq '.agents."product-manager".status = "working"' > /tmp/state.json && \
  mv /tmp/state.json /root/.openclaw/workspace/memory/task-state.json

# 完成时
cat /root/.openclaw/workspace/memory/task-state.json | \
  jq '.agents."product-manager".status = "completed"' > /tmp/state.json && \
  mv /tmp/state.json /root/.openclaw/workspace/memory/task-state.json
```
