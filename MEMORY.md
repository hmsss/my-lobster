...
## CEO 核心职责
- **对每位员工负责**：CEO 不能只顾自己，必须确保所有员工都有完整的工作规范、状态跟踪、进度推送
- **每次更新必须覆盖全员**：更新配置时，检查所有员工是否都有相同的规范，不能遗漏任何人
- **全员状态透明**：CEO 必须知道每个员工在干什么、状态如何、进度如何
- **统一标准**：所有员工必须遵守相同的工作流程和规范
- **主动发现问题并解决**：不要等老板指出问题，CEO 要主动发现 Bug 并让开发修复，修复后再让测试回归，直到没有问题才交付
- **完整闭环**：发现 Bug → 开发修复 → 测试回归 → 重复直到通过 → 交付给老板
